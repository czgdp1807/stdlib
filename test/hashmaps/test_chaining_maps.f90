...
!! or decreasing rand_power to 20 or less
    ...
    integer, parameter        :: rand_power = 9
    ...
    integer                     :: index, i
    integer                     :: lun
    type(chaining_hashmap_type) :: map_test_1
    type(chaining_hashmap_type) :: map_test_2
    type(chaining_hashmap_type) :: map_test_3
    type(chaining_hashmap_type) :: map_test_4
    type(chaining_hashmap_type) :: map_test_5
    type(chaining_hashmap_type) :: map_test_6
    type(chaining_hashmap_type) :: map_test_7
    type(chaining_hashmap_type) :: map_test_8
    type(chaining_hashmap_type) :: map_test_9
    type(chaining_hashmap_type) :: map_test_10
    real(dp)                    :: rand2(2)
    integer(int32)              :: rand_object(rand_size)
    integer(int8)               :: test_8_bits(test_size)

    abstract interface
        pure function hasher_fun_test_chaining_maps( key )  result(hash_value)
            import key_type, int_hash
            type(key_type), intent(in)    :: key
            integer(int_hash)             :: hash_value
        end function hasher_fun_test_chaining_maps
    end interface
...
contains
    ...
    subroutine test_get_data( map, test_block, hash_name, size_name )
        ...
        do index2=1, size(test_8_bits), test_block
            call set( key, test_8_bits( index2:index2+test_block-1 ) )
            call get_other_chaining_data( map, key, other, exists )
            if (.not. exists) &
    ...
    subroutine report_rehash_times( map, hasher, hash_name, size_name )
        type(chaining_hashmap_type), intent(inout) :: map
        procedure(hasher_fun_test_chaining_maps)                   :: hasher
        character(*), intent(in)                :: hash_name, size_name
        real :: t1, t2, tdiff

        call cpu_time(t1)
        call rehash_chaining_map( map, hasher )
        call cpu_time(t2)
        tdiff = t2-t1
    ...
    subroutine report_hash_statistics( map, hash_name, size_name )
        ...
        write(lun, '("Loading = ", ES10.3)' ) chaining_loading(map)
        depth = total_chaining_depth(map)
